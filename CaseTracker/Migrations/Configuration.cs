namespace CaseTracker.Migrations
{
    ...
    internal sealed class Configuration : DbMigrationsConfiguration<CaseTracker.Repository.ApplicationDbContext>
    {
        ...
        protected override void Seed(CaseTracker.Repository.ApplicationDbContext context)
        {
            ...
            {
                Result = "dr_Anepidoto_Anamoni_Neas_Address",
                IsPayable = false
            });

            context.DeedResults.AddOrUpdate(new DeedResult()
            {
                Result = "dr_Anepidoto_Apeviose",
                IsPayable = false
            });

            context.DeedResults.AddOrUpdate(new DeedResult()
            {
                Result = "dr_Anepidoto_Katoikos_Eksoterikou",
                IsPayable = false
            });

            context.DeedResults.AddOrUpdate(new DeedResult()
            {
                Result = "dr_Anepidoto_Lathos_Address",
                IsPayable = false
            });

            context.DeedResults.AddOrUpdate(new DeedResult()
            {
                Result = "dr_Apli_Thirokolisi",
                IsPayable = true
            });
            ...
            context.DeedResults.AddOrUpdate(new DeedResult()
            {
                Result = "dr_Mataiosi_AM_Apli_Thirokolisi",
                IsPayable = false
            });

            context.DeedResults.AddOrUpdate(new DeedResult()
            {
                Result = "dr_Mataiosi_AM_Epidosi",
                IsPayable = false
            });

            context.DeedResults.AddOrUpdate(new DeedResult()
            {
                Result = "dr_Mataiosi_AM_Pliris_Thirokolisi",
                IsPayable = false
            });

            context.DeedResults.AddOrUpdate(new DeedResult()
            {
                Result = "dr_Pausi_Ergasion",
                IsPayable = false
            });

            context.DeedResults.AddOrUpdate(new DeedResult()
            {
                Result = "dr_Pliris_Thirokolisi",
                IsPayable = true
            });
